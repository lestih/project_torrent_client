...
Message Message::Parse(const std::string& messageString) { //todo
    ...
    MessageId id_message = static_cast<MessageId>(messageString[0]);
    std::string data = messageString.substr(1, messageString.size() - 1);
    return {id_message, messageString.size(), data};
}
