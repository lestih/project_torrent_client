...
bool RunDownloadMultithread(PieceStorage& pieces, const TorrentFile& torrentFile, const std::string& ourId, const TorrentTracker& tracker) {
    ...
    while (pieces.PiecesSavedToDiscCount() < (PiecesToDownload * procent) / 100) { //check
...
void TestTorrentFile(const fs::path& file) {
    ...
    try {
        ...
        PiecesToDownload = (torrentFile.length + torrentFile.pieceLength - 1) / torrentFile.pieceLength;
    ...
    PieceStorage pieces(torrentFile, outputDirectory, (PiecesToDownload * procent) / 100);
